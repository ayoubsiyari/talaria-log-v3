...
const EditCampaignModal = ({ isOpen, onClose, onSuccess, campaign }) => {
  ...
  useEffect(() => {
    if (campaign && isOpen) {
      setFormData({
        ...
        usageLimit: campaign.usage_limit?.toString() || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/admin/promotions/${campaign.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        toast.error(errorData.error || `Failed to update campaign (Status: ${response.status})`);
      }
    } catch (error) {
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the promotional campaign details, settings, and scheduling.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="p-4 border rounded-lg bg-muted/50">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <Badge 
                    variant={formData.status === 'active' ? 'default' : formData.status === 'scheduled' ? 'secondary' : formData.status === 'expired' ? 'destructive' : 'outline'}
                    className="ml-2"
                  >
                    {formData.status}
                  </Badge>
